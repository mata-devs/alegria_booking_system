...
import { Check, Image as ImageIcon, Loader2, Sparkles, TriangleAlert } from 'lucide-react';
...
type TabId = 'hero' | 'ticker' | 'preview';
...
export default function HomepageCmsPage() {
  ...
  useEffect(() => {
    const unsub = subscribeHomepageCms(
      ...
      (err) => {
        ...
        setLoadError('Could not load Homepage CMS. Check your permissions and try again.');
  ...
  const dirty = useMemo(() => {
    ...
    return (
      JSON.stringify({ hero: server.hero, ticker: server.ticker }) !==
      JSON.stringify({ hero: draft.hero, ticker: draft.ticker })
    );
  }, [server, draft]);
  ...
  const validationError = useMemo(() => {
    ...
    if (draft.ticker.items.length === 0) {
      return 'Add at least one municipality before saving.';
    }
    if (draft.ticker.items.some((i) => !i.bestPictureUrl)) {
      return 'Every municipality needs a best picture.';
    }
    return null;
  }, [draft]);
  ...
  const onSave = async () => {
    ...
    try {
      await saveHomepageCms(
        { hero: draft.hero, ticker: draft.ticker },
        authState.user.uid,
      );
      setStatus({ type: 'success', msg: 'Homepage CMS updated.' });
    } catch (err) {
      setStatus({
        type: 'error',
        msg: err instanceof Error ? err.message : 'Failed to save Homepage CMS.',
      });
    }
  };
  ...
  return (
    <div className="-m-6 min-h-[calc(100vh-0px)] bg-gray-50">
      <header className="border-b border-gray-200 bg-white px-6 py-5">
        ...
        <h1 className="mt-1 text-2xl font-bold leading-tight text-gray-900">Homepage</h1>
        <p className="mt-1 text-xs text-gray-500">
          Curate the magnific-style municipality ticker and hero copy shown on the guest landing
          page.
        </p>
      </header>
              ...
              >
                ...
                {tab.id === 'ticker' && draft && (
                  <span className="rounded-full bg-gray-100 px-1.5 text-[10px] font-semibold text-gray-600">
                    {publishedCount}/{draft.ticker.items.length}
                  </span>
                )}
              </button>
            );
