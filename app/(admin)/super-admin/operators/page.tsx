        ...
        <div className="mt-4 flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1 min-w-0 rounded-lg border border-gray-200 bg-white p-4">
                ...
                {dropdownOpen && (
                  <div className="absolute left-0 top-full z-10 mt-1 w-44 rounded-md border border-gray-200 bg-white py-1 shadow-lg">
                    {(Object.keys(SEARCH_FIELD_LABELS) as SearchField[]).map((field) => (
                      <button
                        key={field}
                        onClick={() => { setSearchField(field); setDropdownOpen(false); }}
            ...
            <div className="mt-3 flex flex-col gap-2">
              {loading ? (
                <div className="rounded-lg bg-gray-100 px-4 py-4 text-center text-sm text-gray-400">Loading operators…</div>
              ) : filtered.length === 0 ? (
                <div className="rounded-lg bg-gray-100 px-4 py-4 text-center text-sm text-gray-400">No operators found.</div>
              ) : (
              ...
              <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {Array.from({ length: operatorTotalPages }, (_, i) => i + 1).map((page) => (
                    <button key={page} onClick={() => setOperatorPage(page)} className={`h-7 min-w-7 rounded-md px-2 text-xs font-medium transition-colors ${operatorPage === page ? 'bg-[#558B2F] text-white' : 'text-gray-600 hover:bg-gray-100'}`}>
                      {page}
                    </button>
                  ))}
            ...
            <div className="w-full lg:w-80 shrink-0 rounded-lg border border-gray-200 bg-white p-5">
              ...
              <div className="mt-4 flex gap-4">
                <div className="h-24 w-24 shrink-0 overflow-hidden rounded-md bg-gray-200">
                  {selectedOperator.profileImage ? (
                    <Image src={selectedOperator.profileImage} alt={`${selectedOperator.firstName} ${selectedOperator.lastName}`} className="h-full w-full object-cover" width={96} height={96} />
                  ) : (
              ...
              <div className="mt-5">
                ...
                <div className="relative mt-1 inline-block">
                  <button onClick={() => setStatusDropdownOpen((o) => !o)} disabled={statusUpdating} className="inline-flex items-center gap-1.5 rounded-full border border-gray-300 px-4 py-1.5 text-sm hover:bg-gray-50 transition-colors disabled:opacity-50">
                    <span className={selectedOperator.status === 'active' ? 'text-green-600 font-medium' : 'text-red-500 font-medium'}>
                  ...
                  {statusDropdownOpen && (
                    <div className="absolute left-0 top-full z-20 mt-1 w-36 rounded-md border border-gray-200 bg-white py-1 shadow-lg">
                      <button onClick={() => updateOperatorStatus(selectedOperator.uid, 'active')} className={`flex w-full items-center gap-2 px-4 py-2 text-left text-sm hover:bg-gray-100 ${selectedOperator.status === 'active' ? 'text-green-600 font-medium' : 'text-gray-700'}`}>Active</button>
                      <button onClick={() => updateOperatorStatus(selectedOperator.uid, 'suspended')} className={`flex w-full items-center gap-2 px-4 py-2 text-left text-sm hover:bg-gray-100 ${selectedOperator.status === 'suspended' ? 'text-red-500 font-medium' : 'text-gray-700'}`}>Suspended</button>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="flex flex-1 min-w-0 flex-col gap-4">
            <div className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className="text-sm font-medium text-gray-700">Search by:</span>
                <div className="relative">
                  <button onClick={() => setRequestDropdownOpen(!requestDropdownOpen)} className="inline-flex items-center gap-1.5 rounded-full border border-[#558B2F] px-4 py-1.5 text-sm font-medium text-[#558B2F] hover:bg-green-50 transition-colors">
                  ...
                  {requestDropdownOpen && (
                    <div className="absolute left-0 top-full z-10 mt-1 w-44 rounded-md border border-gray-200 bg-white py-1 shadow-lg">
                      {(Object.keys(REQUEST_SEARCH_LABELS) as RequestSearchField[]).map((field) => (
                        <button key={field} onClick={() => { setRequestSearchField(field); setRequestDropdownOpen(false); }} className={`block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 ${requestSearchField === field ? 'text-[#558B2F] font-medium' : 'text-gray-700'}`}>
                ...
                <div className="relative">
                  <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                  <input type="text" placeholder="Search" value={requestSearchQuery} onChange={(e) => setRequestSearchQuery(e.target.value)} className="rounded-md border border-gray-300 py-1.5 pl-9 pr-4 text-sm text-gray-700 placeholder:text-gray-400 focus:border-[#558B2F] focus:outline-none focus:ring-1 focus:ring-[#558B2F]" />
                </div>
              </div>
              ...
              <div className="mt-3 flex flex-col gap-2">
                {requestsLoading ? (
                  <div className="rounded-lg bg-gray-100 px-4 py-4 text-center text-sm text-gray-400">Loading requests…</div>
                ) : filteredRequests.length === 0 ? (
                  <div className="rounded-lg bg-gray-100 px-4 py-4 text-center text-sm text-gray-400">No sign-up requests found.</div>
                ) : (
            ...
            <div className="w-full lg:w-[22rem] shrink-0 rounded-lg border border-gray-200 bg-white p-5">
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-0.5 rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-900 truncate">{selectedRequest.name || '—'}</div>
                  <label className="mt-3 block text-xs font-semibold text-gray-700">Telephone number</label>
                  <div className="mt-0.5 rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-900">{selectedRequest.phoneNumber || '—'}</div>
                </div>
              </div>
              ...
              {selectedRequest.status === 'pending' && (
                <div className="mt-6 flex gap-3">
                  ...
                  <button disabled={!!requestUpdating} onClick={() => updateRequestStatus(selectedRequest.id, 'rejected')} className="flex-1 rounded-lg border border-red-300 py-2.5 text-sm font-semibold text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50">
