...
export function PaymentInstructions({
    ...
    const handleDownload = async () => {
        ...
        try {
            // Same-origin proxy to avoid Firebase Storage CORS issues + force attachment download.
            const downloadUrl =
                `/api/payment-image?url=${encodeURIComponent(paymentImageUrl)}` +
            ...
            // If download fails (rare), do nothing instead of opening a new tab.
            // (Browsers often block cross-origin downloads without proper CORS.)
        }
    };
    ...
    return (
        <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <div className="mt-5 rounded-2xl border border-gray-200 bg-white px-4 py-4">
                ...
                <div ref={dropdownRef} className="relative mt-2 flex items-center justify-between gap-3">
                    ...
                    <button
                        ...
                        onClick={() => {
                            if (!canEditMethod) return;
                            setShowMethodDropdown((p) => !p);
                        }}
                    ...
                    >
                        Change
                        {canEditMethod && (
                            <ChevronDown className={`ml-2 h-4 w-4 text-gray-400 transition-transform ${showMethodDropdown ? "rotate-180" : ""}`} />
                        )}
                    </button>
                            ...
                            {PAYMENT_METHODS.map((m) => (
                                <button
                                    ...
                                    className={`w-full px-4 py-3 text-left text-sm font-semibold transition ${
                                        paymentMethod === m ? "bg-[#f0fce0] text-[#74C00F]" : "text-gray-900 hover:bg-gray-50"
                                    }`}
                                >
                                    {m}
                                </button>
            ...
            <div className="mt-6">
                <p className="text-sm font-bold text-gray-900">Payment Instruction</p>
                    ...
                    <li className="flex gap-2">
                        ...
                        <span>
                            ...
                            {typeof totalAmount === "number" ? (
                                <span className="font-bold text-gray-900">({peso(totalAmount)})</span>
                            ) : null}
                            .
                        </span>
                    </li>
                    ...
                    {paymentImageUrl ? (
                        <>
                            ...
                            <button
                                ...
                                className="absolute bottom-3 right-3 inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/30 bg-black/55 text-white shadow-lg shadow-black/20 backdrop-blur-sm transition
                                hover:bg-black/70 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#74C00F] focus-visible:ring-offset-2 focus-visible:ring-offset-white
                        ...
                        <div className="p-8 text-center text-sm text-gray-500">
                            No payment instruction image is configured for this operator yet.
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="mt-4 grid gap-3">
                ...
                <div className="rounded-2xl border border-gray-200 bg-white px-4 py-3">
                    ...
                    <p className="mt-1 break-words font-mono text-sm font-bold text-gray-900">
                        {accountNumber ?? "—"}
                    </p>
                </div>
            </div>
