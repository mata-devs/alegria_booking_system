...
interface GuestsListProps {
    ...
    /** External errors per guest index, passed down from page-level submit validation */
    guestErrors?: Record<number, Partial<Record<string, string>>>;
    /** Set to true once user has tried to submit */
    submitted?: boolean;
}

// ─── Helpers ──────────────────────────────────────────────────────────────────

/** Validate a single guest field; returns the first error message or undefined */
...
const GuestRow = ({ idx, guest, onUpdate, onRemove, externalErrors = {}, submitted }: GuestRowProps) => {
    ...
    const handleBlur = useCallback((field: string, value: string) => {
        markTouched(field);
        const msg = validateGuestField(field, value);
        setLiveErrors((prev) => ({ ...prev, [field]: msg }));
    }, [markTouched]);

    const handleChange = useCallback((field: string, value: string) => {
        onUpdate(field, value);
        // Always mark touched on any change so the error clears immediately
        markTouched(field);
        const msg = validateGuestField(field, value);
        setLiveErrors((prev) => ({ ...prev, [field]: msg }));
    ...
    // This ensures fixing a field immediately clears its error.
                ...
                <CountryDropdown
                    ...
                    onChange={(val) => {
                        ...
                        if (touched["nationality"] || submitted) {
                            const msg = validateGuestField("nationality", val);
                            setLiveErrors((prev) => ({ ...prev, nationality: msg }));
                        }
...
export const GuestsList = ({
    ...
    return (
        <div className="mb-8">
            ...
            {/* Empty state or Add another guest link */}
            {guests.length === 0 ? (
                <button
                    type="button"
                    onClick={onAddGuest}
