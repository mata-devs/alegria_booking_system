...
export const CountryDropdown = ({
    ...
    const selectedCountry = useMemo(
        () => activeCountries.find((c) => c.alpha2 === selectedAlpha2),
        ...
        <div className="relative" ref={dropdownRef}>
            ...
            {open && (
                <div className="absolute z-50 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-xl max-h-[16rem] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="overflow-y-auto w-full py-1 h-full">
                        {filteredCountries.length > 0 ? (
                            filteredCountries.map((country) => (
                                <button
                                    ...
                                    className={`w-full text-left flex items-center justify-between gap-3 px-3 py-2.5 hover:bg-[#F5FFE6] transition text-sm ${
                                        selectedAlpha2 === country.alpha2
                                            ? "bg-[#F5FFE6] font-medium text-[#74C00F]"
                                            : "text-gray-700"
                                    }`}
                                >
                                    <div className="flex items-center gap-3 truncate">
