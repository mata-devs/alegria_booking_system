...
export function UploadPayment({
    ...
    const handleFileInput = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file) { onFileChange(event); return; }
    ...
    const backHref = {
        ...
        query: {
            ...
            ...(draft.activityName ? { activityName: draft.activityName } : {}),
    ...
    return (
        <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <label className="mt-6 flex min-h-[12rem] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-gray-200 bg-[#F9FAFB] px-6 py-8 transition hover:border-[#74C00F]/50 hover:bg-white md:min-h-[14rem] md:py-10">
                {previewUrl ? (
                    <>
                        <div className="relative h-40 w-full max-w-md md:h-44">
                            <Image src={previewUrl} alt="Payment screenshot preview" fill unoptimized className="object-contain" sizes="(max-width: 768px) 100vw, 28rem" />
                        </div>
                ...
                <input type="file" accept=".jpg,.jpeg,.png,image/jpeg,image/png" onChange={handleFileInput} className="sr-only" />
            </label>
