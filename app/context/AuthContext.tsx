...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const verifyAndSetAuthState = useCallback(async (firebaseUser: User) => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(firebaseAuth, async (firebaseUser) => {
      ...
      await verifyAndSetAuthState(firebaseUser);
    });
  ...
  async function retryAuth() {
    const currentUser = authState.status === 'authenticated' || authState.status === 'unauthorized'
      ? authState.user
    ...
    await verifyAndSetAuthState(currentUser);
  }

  return (
    <AuthContext.Provider value={{ authState, signOutUser, retryAuth }}>
      {children}
