 ...
 *
 * Pass `disableAutoCarousel: true` to prevent hover-based auto-rotation.
 */
export function useImageCarousel(
  images: string[] | undefined,
  fallback: string,
  disableAutoCarousel = false
) {
  ...
  useEffect(() => {
    if (disableAutoCarousel || !isHovered || !hasMultiple) return
  ...
  }, [disableAutoCarousel, isHovered, hasMultiple, imgList])
